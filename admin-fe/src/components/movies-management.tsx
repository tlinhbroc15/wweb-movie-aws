...
async function uploadFile(
  ...
  try {
    ...
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/aws/upload`, formData, {
...
export function MoviesManagement() {
  ...
  const handleAddMovie = async (newMovie: Omit<Movie, "id">) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/movies`, JSON.stringify(movieData), {
        headers: {
    ...
    try {
      ...
      if (movieToDelete) {
        if (typeof movieToDelete.poster === "string") {
          ...
          await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/aws/file`, {
...
export function EpisodeManager({
  ...
  const fetchEpisodes = useCallback(async () => {
    try {
      if (!session?.data?.token) return
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/episodes/movie/${movieId}`, {
        headers: {
          Authorization: `Bearer ${session.data.token}`,
        },
      })
  ...
  const handleAddEpisode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/episodes`,
        {
          movieId,
          videoUrl: finalUrl,
  ...
  const handleDeleteEpisode = async (episodeId: number) => {
    try {
      if (!session?.data?.token) return
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/episodes/${episodeId}`, {
        headers: {
          Authorization: `Bearer ${session.data.token}`,
        },
      })
